 ...
 * ```
 */
View.prototype.getTypedArray = function (path, sab) {
    ...
    return new globalThis[view.type](sab, view.offset + 4, view.length);
};
/**
 ...
 * ```
 */
View.prototype.exportSchema = function () {
 ...
 * ```
 */
View.importSchema = function (schemaJson, byteSize) {
    if (byteSize < 8)
        throw new Error('Byte size must be at least 8');
    let parsed;
    try {
    ...
    const schema = {
        ...
        encodingTemplate: parsed.encodingTemplate.map((t) => ({
            ...t,
    ...
    const view = new View({}, { mutable: true });
    view.schema = schema;
 ...
 * ```
 */
View.prototype.schemaUpdate = function (path, value, sab) {
