 ...
 * ```
 */
export function compileFunctions(schema) {
     ...
     */
    const encode = (data, dv, dirtyFields) => {
        for (const entry of encodingTemplate) {
            if (dirtyFields && !dirtyFields.has(entry.type))
                continue;
            const value = getValueByPath(data, entry.type);
            if (value === undefined)
                throw new Error(`Missing value at path: ${entry.type}`);
 ...
 */
function traverseDescriptor(descriptor, dv, offset, result, path) {
 ...
 */
function readPrimitive(dv, offset, dataType) {
    if (offset < 0 || offset >= dv.byteLength)
        throw new Error(`Invalid offset: ${offset}`);
    switch (dataType) {
 ...
 */
function compareValues(path, oldValue, newValue, descriptor, patches) {
