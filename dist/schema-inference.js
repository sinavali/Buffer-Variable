...
export function inferSchema(data) {
    ...
    const encodingTemplate = [];
    const typedArrayViews = new Map();
    /**
     ...
     */
    function inferDescriptor(value, path) {
        if (value === null) {
            return {
                type: 'Primitive',
                dataType: 'Null',
        ...
        if (typeof value === 'number') {
            ...
            encodingTemplate.push({
                ...
                type: path,
                size: 8,
                setter: (dv, v) => dv.setFloat64(currentOffset, v, true),
                getter: (dv) => dv.getFloat64(currentOffset, true),
            });
        ...
        if (typeof value === 'boolean') {
            ...
            encodingTemplate.push({
                ...
                type: path,
                size: 1,
                setter: (dv, v) => dv.setUint8(currentOffset, v ? 1 : 0),
                getter: (dv) => dv.getUint8(currentOffset) !== 0,
            });
            const offset = currentOffset;
            currentOffset += 1;
            return {
        ...
        if (typeof value === 'bigint') {
            ...
            encodingTemplate.push({
                ...
                type: path,
                size: 8,
                setter: (dv, v) => dv.setBigInt64(currentOffset, v, true),
                getter: (dv) => dv.getBigInt64(currentOffset, true),
            });
            const offset = currentOffset;
            currentOffset += 8;
            return {
        ...
        if (value instanceof Date) {
            ...
            encodingTemplate.push({
                ...
                type: path,
                size: 8,
                setter: (dv, v) => dv.setBigInt64(currentOffset, BigInt(v.getTime()), true),
                getter: (dv) => new Date(Number(dv.getBigInt64(currentOffset, true))),
            });
            const offset = currentOffset;
            currentOffset += 8;
            return {
        ...
        if (isTypedArray(value)) {
            ...
            encodingTemplate.push({
                ...
                setter: (dv, v) => {
                    ...
                    new globalThis[type](dv.buffer, currentOffset + 4, v.length).set(v);
                },
                getter: (dv) => {
                    const len = dv.getUint32(currentOffset, true);
                    return new globalThis[type](dv.buffer, currentOffset + 4, len);
                },
            });
...
function getTypedArrayByteSize(type) {
    ...
    if (!size)
        throw new Error(`Invalid typed array type: ${type}`);
    return size;
}
